...
def print_planets_nakshatras(tjd=JulianDay(), ayanamsa=pglob.ayanamsa):
    ...
    if ayanamsa == 98:
        ...
        print("My Dhruva GC mid-Mula ecliptic coordinates non-ayanamsa")
    else:
...
def print_vimshottari_dasha(panch=Panchanga(),dlevels=1,yrlen=pglob.saura_year):
...
#    age += dashas[first_dasha%9][length]
    dasha_starts = panch.shift('b','d',-age*yrlen)
    level = 0
    for d in range(0,9):
