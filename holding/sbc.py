...
def main():
    ...
    if ttype == 1:
        d.append(
            draw.Text(
                str((btithi - 15) % 15),
                font_size=10,
                x=coords[thisx][thisy][0] + 10,
                y=coords[thisx][thisy][1] + 14,
            )
        )
    if ttype == 2:
        d.append(
    ...
    if ttype == 4:
        d.append(
            draw.Text(
                "15" if (btithi - 15) % 15 == 0 else str((btithi - 15) % 15),
                font_size=10,
                x=coords[4][4][0] + 10,
                y=coords[4][4][1] + 17,
    ...
    d.append(
        draw.Rectangle(35, 450, 70, 45, rx="1", ry="1", stroke="black", fill="yellow")
    )
    ...
    d.append(
        draw.Rectangle(400, 0, 100, 45, rx="1", ry="1", stroke="black", fill="yellow")
    )
    d.append(draw.Text(bntstr, font_size=7.5, x=405, y=10))

    d.append(
        draw.Rectangle(0, 0, 100, 50, rx="1", ry="1", stroke="black", fill="yellow")
    )
    ...
    d.append(
        draw.Text(
            sc.moon, font_size=15, x=coords[my][mx][0] + 10, y=coords[my][mx][1] + 15
        )
    )
    ...
    d.append(
        draw.Text(
            sc.lagna, font_size=10, x=coords[ly][lx][0] + 10, y=coords[ly][lx][1] + 15
        )
    )
    ...
    poffsets = {
        1: [15, [(10, 15)]],
        2: [12, [(2, 15), (15, 15)]],
        3: [10, [(5, 15), (20, 25), (15, 15)]],
        4: [10, [(5, 10), (20, 25), (20, 10), (11, 19)]],
        5: [10, [(5, 10), (20, 25), (20, 10), (5, 19), (15, 19)]],
        6: [7, [(5, 8), (20, 25), (20, 8), (5, 19), (12, 19), (12.5, 8)]],
        7: [7, [(5, 8), (20, 25), (20, 8), (5, 19), (12, 19), (12.5, 8), (20, 19)]],
        8: [
            7,
            [
                (5, 8),
    ...
    for n in range(len(bnaks)):
        ...
        d.append(
            draw.Text(
                sc.pglyphs[n],
                font_size=poffsets[num][0],
                x=coords[ny][nx][0] + (poffsets[num][1][nakcount[bnaks[n]]][0]),
                y=coords[ny][nx][1] + (poffsets[num][1][nakcount[bnaks[n]]][1]),
            )
        )
        nakcount[bnaks[n]] += 1
    ...
    tplanets = init_Planets(transit_ephtime)
    tcusps = Cusps(
        sc.hsys, Location(lat=tlat, long=tlong, placename=tplace), transit_ephtime
    )
    tcusps.init_cusps()
    ...
    toffright = {
        n: tright[:n] for n in range(1, len(tright) + 1)
    }  # a quick way of writing it
    tbot = [(2, 42), (20, 42), (2, 57), (20, 57), (2, 72), (20, 72), (2, 87), (20, 87)]
    toffbot = {
        n: tbot[:n] for n in range(1, len(tbot) + 1)
    }  # a quick way of writing it
    tleft = [
        (-18, 13),
        (-18, 27),
    ...
    toffleft = {
        n: tleft[:n] for n in range(1, len(tleft) + 1)
    }  # a quick way of writing it
        ...
        if nrow == 0:  # the row of seven nakshatras, krtikkikadi
            d.append(
                draw.Text(
                    sc.pglyphs[n],
                    font_size=15,
                    x=coords[ncol][nrow][0] + (toffup[num][nakcount[tnaks[n]]][0]),
                    y=coords[ncol][nrow][1] + (toffup[num][nakcount[tnaks[n]]][1]),
                )
            )
        if nrow == 8:  # the bottom of of seven nakshatras
            d.append(
                draw.Text(
                    sc.pglyphs[n],
                    font_size=15,
                    x=coords[ncol][nrow][0] + (toffbot[num][nakcount[tnaks[n]]][0]),
                    y=coords[ncol][nrow][1] + (toffbot[num][nakcount[tnaks[n]]][1]),
                )
            )
        if ncol == 0:  # the first column of nakshatras, i.e., ending with bharani
            d.append(
                draw.Text(
                    sc.pglyphs[n],
                    font_size=15,
                    x=coords[ncol][nrow][0] + (toffleft[num][nakcount[tnaks[n]]][0]),
                    y=coords[ncol][nrow][1] + (toffleft[num][nakcount[tnaks[n]]][1]),
                )
            )
        if ncol == 8:  # the rightmost column of nakshatras
            d.append(
                draw.Text(
                    sc.pglyphs[n],
                    font_size=15,
                    x=coords[ncol][nrow][0] + (toffright[num][nakcount[tnaks[n]]][0]),
                    y=coords[ncol][nrow][1] + (toffright[num][nakcount[tnaks[n]]][1]),
                )
            )
        nakcount[tnaks[n]] += 1
    ...
    if planet_info:
        ...
        pprint.print_Cusps(
            loc=Location(lat=blat, long=blong, placename=bplace), tjd=bephtime
        )
        ...
        pprint.print_Cusps(
            loc=Location(lat=tlat, long=tlong, placename=tplace), tjd=transit_ephtime
        )
...
def get_args():
    ...
    parser.add_argument(
        "-l",
        "--lang",
        help="language file; options: dict.eng, dict.iast, dict.deva, dict.mixed",
    )
    parser.add_argument(
        "-o",
        "--output-file",
