...
class Nakshatra:
    ...
    def print_in_longitude(self):
        print(
            f"Elapsed: {self.degrees_elapsed()} deg ({round((self.degrees_elapsed() / self.naksize()) * 100, 3)} %)"
        )
        ...
        for occupant in self.nakshatras:
            output.add_row(
                [
                    occupant.occupant(),
                    occupant.nakshatra(),
                    f"{occupant.degrees_elapsed()} ({occupant.percent_elapsed()} %)",
                ]
            )

        ret = output.get_string(
            fields=[f"{self.occupant_type()}", "Nakshatra", "Percent Elapsed"]
        )
