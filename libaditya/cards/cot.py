...
class CoT:
    ...
    def quadrate(tquad):
        ...
        quad = (
            tquad.copy()
        )  # so that i dont have to pass a copy of the quadration to this function
    ...
    def getbspreadwxcfromquad(card, pos, quad):
        ...
        cindex = quad.index(
            getindex(card)
        )  # this is where the card is in the quadration
        ...
        if cindex - pos >= 0:
            return CoT.get_birthspread_from_quadration(
                cardsc.cards[quad[cindex - pos]], quad
            )
        else:
            return CoT.get_birthspread_from_quadration(
                cardsc.cards[quad[52 - (cindex - pos)]], quad
            )
