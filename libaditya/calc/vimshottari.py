...
def print_vimshottari_dasha(planet=Moon(), dlevels=1, yrlen=365.2422):
    ...
    for d in range(0, 9):
        ...
        print(
            f"Duration: {const.vimshottari_dashas[(first_dasha + d) % 9][length]} years"
        )
        ...
        dasha_starts = dasha_starts.shift(
            "f", "d", (const.vimshottari_dashas[(first_dasha + d) % 9][length]) * yrlen
        )
...
def print_next_dasha_level(dlist, dasha_time, level, dlevels, yrlen, age):
    ...
    if level <= dlevels:
        ...
        for d in range(0, 9):
            ...
            if level + 1 < dlevels:
                print_next_dasha_level(
                    dlist + [this_dasha], dasha_time, level + 1, dlevels, yrlen, age
                )
...
def calculate_vimshottari_dasha(
    planet=Moon(), dlevels=1, yrlen=const.dasha_years["saura"]
):
    """
...
def calculate_specific_period(
    planet=Moon(), lords=[], yrlen=const.dasha_years["saura"]
):
    """
...
def calc_vdasha(dlist, dasha_time, level, dlevels, yrlen):
    ...
    if level + 1 <= dlevels:
        ...
        for d in range(0, 9):
            ...
            if level + 1 < dlevels:
                next_dasha_list = calc_vdasha(
                    dlist + [this_dasha], dasha_time, level + 1, dlevels, yrlen
                )
...
def current_vimshottari_dasha(
    planet=Moon(), nowtimeJD=JulianDay(), dlevels=3, yrlen=const.dasha_years["saura"]
):
    """
                    ...
                    if get_next_lord(next_next_lord) == next_next_next_lord:
                        if length > 4:
                            ...
                            if get_next_lord(next_next_next_lord) == mahadasha_lord:
                                ...
                                lords[0] = lords[1] = lords[2] = lords[3] = lords[4] = (
                                    mahadasha_lord
                                )
                            else:
                                next_next_next_lord = get_next_lord(next_next_next_lord)
                                lords[0] = lords[1] = lords[2] = lords[3] = (
                                    next_next_next_lord
                                )
                    else:
...
def print_current_vdasha(context, yrlen, levels):
    ...
    for level in levels:
        ...
        print(
            f"Next dasha: {utils.mk_dasha_lord(next_dasha_lords(current_dasha))}, starts at:"
        )
        print(f"{next_dasha_startsJD}")
        print(
            f"in {utils.dec2ymd((next_dasha_startsJD.jd_number() - now.jd_number()) / 365.2422)}\n"
        )
    return
