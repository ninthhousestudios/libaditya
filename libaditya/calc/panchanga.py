...
class Panchanga:
    ...
    def __init__(self, context=EphContext()):
        ...
        self._tithi_number, self._tithi_elapsed, self._tithi_remaining = (
            self.init_tithi()
        )
        self._karana_number, self._karana_elapsed, self._karana_remaining = (
            self.init_karana()
        )
    ...
    def init_tithi(self):
        traw = (
            (self._moon.ecliptic_longitude() - self._sun.ecliptic_longitude()) % 360
        ) / 12
    ...
    def karana(self):
        return const.names[self.context.names_type]["karanas"][self.karana_index()[0]][
            self.karana_index()[1]
        ]
    ...
    def vara(self):
        ...
        # there is this nice function in the seam actually...
        # but for it Monday = 0 and Sunday = 6
        # weekday = seam.day_of_week(self.context.timeJD.jd_number())
    ...
    def init_yoga(self):
        ...
        yraw = (
            (
                self._moon.nakshatra().ashvini_longitude()
                + self._sun.nakshatra().ashvini_longitude()
            )
            % 360
        ) / (13 + (20 / 60))
    ...
    def next_tithi(self) -> Self:
        if self.tithi_degrees_remaining() == 0:
            return Panchanga(
                replace(self.context, timeJD=self.timeJD.shift("f", "seconds", 5))
            )
    ...
    def next_karana(self) -> Self:
        ...
        shift_factor = (
            self.karana_degrees_remaining() * self._moon.lowest_hourly_speed()
        )
        return Panchanga(
            replace(self.context, timeJD=self.timeJD.shift("f", "hours", shift_factor))
        ).next_karana()
    ...
    def next_vara(self) -> Self:
        ...
        if self.timeJD.jd_number() < today_yamakotiJD.jd_number():
            return Panchanga(
                replace(self.context, timeJD=today_yamakotiJD.shift("f", "minutes", 5))
            )
        # if the current time is after sunrise at yamaktoi, go ahead one today
        next_yamakotiJD = Sun(
            replace(self.context, timeJD=self.timeJD.shift("f", "day", 1))
        ).sunrise_yamakoti()
        return Panchanga(
            replace(
                self.context, timeJD=next_yamakotiJD.timeJD.shift("f", "minutes", 5)
            )
        ).next_vara()

    def next_nakshatra(self):
        if self._moon.nakshatra().degrees_remaining() <= 0.00001:
            return Panchanga(
                replace(self.context, timeJD=self.timeJD.shift("f", "minute", 1))
            )
        shift_factor = (
            self._moon.nakshatra().degrees_remaining()
            * self._moon.lowest_hourly_speed()
        )
        return Panchanga(
            replace(self.context, timeJD=self.timeJD.shift("f", "hours", shift_factor))
        ).next_nakshatra()
    ...
    def shift(self, dir, tunit, shift_factor):
        return Panchanga(
            replace(self.context, timeJD=self.timeJD.shift(dir, tunit, shift_factor))
        )
    ...
    def next_new_moon(self):
        ...
        if self.tithi() != 30:
            ...
            return Panchanga(
                replace(self.context, timeJD=self.timeJD.shift("f", "hour", 8))
            ).next_new_moon()
        if (self._sun.ecliptic_longitude() - self._moon.ecliptic_longitude()) <= 0.001:
            return Panchanga(
                replace(self.context, timeJD=self.timeJD.shift("f", "second", 10))
            )
    ...
    def print_addendum(self):
        ...
        print(
            "Elapsed: ",
            self.tithi_degrees_elapsed(),
            " degrees (",
            round((self.tithi_degrees_elapsed() / 12) * 100, 3),
            "%)",
        )
        print(
            "Remaining: ",
            self.tithi_degrees_remaining(),
            " degree (",
            round((self.tithi_degrees_remaining() / 12) * 100, 3),
            "%)",
        )
        ...
        print(
            f"Ending time of current tithi: {round(hours_left, 2)} hours from panchanga time"
        )
        print(
            f"Next tithi is {next_tithi.tithi()} ({next_tithi.tithi_type()}), starting at:"
        )
        ...
        print(
            f"Ending time of current karana: {round(hours_left, 2)} hours from panchanga time"
        )
        ...
        print(
            f"Ending time of current vara: {round(hours_left, 2)} hours from panchanga time"
        )
        ...
        print(
            f"Ending time of current nakshatra: {round(hours_left, 2)} hours from panchanga time"
        )
        ...
        print(
            f"Ending time of current yoga: {round(hours_left, 2)} hours from panchanga time"
        )
