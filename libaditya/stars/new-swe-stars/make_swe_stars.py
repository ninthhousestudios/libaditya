...
def main():
    ...
    for star in star_names:
        ...
        if skipped:
            print(
                f"Skipping duplicate names for '{star}': {', '.join(skipped)}",
                file=sys.stderr,
            )
...
def nomen_to_long_form(nomen: str) -> str:
    ...
    if number != "":
        return (
            star_names_short_to_long["constellations"][special] + f" {number.strip()}"
        )
    if special == "VC":
    ...
    if number:
        return (
            star_names_short_to_long["greek"][greek]
            + " "
            + star_names_short_to_long["constellations"][latin]
            + f" {number}"
        )
    return (
        star_names_short_to_long["greek"][greek]
