...
class Location:
    ...
    def listLocations(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/location/list", auth=("", self.password)
        )
        try:
            return r.json()
    ...
    def listCountries(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/location/countrylist",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def listPlanets(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/location/planetlist",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def planetImage(self, planet):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/location/planetimage",
    ...
    def setLocation(
        ...
        if not id == None:
            ...
            if not planet == None:
                params["planet"] = planet
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/location/setlocationfields",
            params=params,
            auth=("", self.password),
        )
        if not r.text == "ok":
