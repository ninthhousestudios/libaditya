...
class Scripts:
    ...
    def listScripts(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/scripts/list", auth=("", self.password)
        )
        try:
            return r.json()
    ...
    def getInfo(self, id, html=False):
        ...
        if html:
            params["html"] = 1
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/scripts/info",
            params=params,
            auth=("", self.password),
        )
        try:
    ...
    def getStatus(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/scripts/status", auth=("", self.password)
        )
        try:
            return r.json()
        except requests.exceptions.JSONDecodeError:
    ...
    def runScript(self, id):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/scripts/run",
            data={"id": id},
            auth=("", self.password),
        )
        if not r.text == "ok":
    ...
    def executeCode(self, code, useIncludes=False):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/scripts/direct",
    ...
    def stopScript(self):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/scripts/stop", auth=("", self.password)
        )
        if not r.text == "ok":
            raise Exception(r.text)
        return r
