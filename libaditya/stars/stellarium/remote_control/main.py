...
class Main:
    ...
    def getStatus(self, format="json"):
        ...
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/main/status",
            params=params,
            auth=("", self.password),
        )
        try:
    ...
    def getPlugins(self):
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/main/plugins", auth=("", self.password)
        )
        try:
            return r.json()
        except requests.exceptions.JSONDecodeError:
    ...
    def getView(self, coord=None, ref=None):
        ...
        if ref is not None:
            params["ref"] = ref
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/main/view",
            params=params,
            auth=("", self.password),
        )
        try:
    ...
    def setTimeJD(self, time):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/main/time",
            data={"time": time},
            auth=("", self.password),
        )
        if not r.text == "ok":
    ...
    def setTimeRate(self, timeRate):
        ...
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/main/time",
            data={"timerate": timeRate / 86400.0},
            auth=("", self.password),
        )
        if not r.text == "ok":
            raise Exception(r.text)
    ...
    def setFocus(self, target=None, position=None, mode=None):
        ...
        if mode is not None:
            params["mode"] = mode
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/main/focus",
            data=params,
            auth=("", self.password),
        )
        if not r.text == "true":
    ...
    def setMove(self, x, y):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/main/move",
            data={"x": x, "y": y},
            auth=("", self.password),
        )
        if not r.text == "ok":
    ...
    def setFov(self, fov):
        ...
        """
        r = requests.post(
            f"http://{self.ip}:{self.port}/api/main/fov",
            data={"fov": fov},
            auth=("", self.password),
        )
        if not r.text == "ok":
