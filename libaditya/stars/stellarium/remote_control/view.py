...
class View:
    ...
    def listLandscape(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/listlandscape",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def landscapeDesciption(self, path):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/landscapedescription/{path}",
            auth=("", self.password),
        )
    ...
    def listSkyculture(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/listskyculture",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def skycultureDescription(self, path):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/skyculturedescription/{path}",
            auth=("", self.password),
        )
        if r.status_code == 200:
            return r.content
    ...
    def listProjection(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/listprojection",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def projectionDescription(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/view/projectiondescription",
            auth=("", self.password),
        )
        if r.status_code == 200:
            return r.content
