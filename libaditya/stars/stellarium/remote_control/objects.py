...
class Objects:
    ...
    def findObject(self, name):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/objects/find",
            params={"str": name},
            auth=("", self.password),
        )
        try:
    ...
    def getInfo(self, name=None, format="json"):
        ...
        if name is not None:
            params["name"] = name
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/objects/info",
            params=params,
            auth=("", self.password),
        )
        if format == "json":
    ...
    def listObjectTypes(self):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/objects/listobjecttypes",
            auth=("", self.password),
        )
        try:
            return r.json()
    ...
    def listObjectsByType(self, type, english=False):
        ...
        """
        r = requests.get(
            f"http://{self.ip}:{self.port}/api/objects/listobjectsbytype",
